    ...
    _format_actions_usage (actions, groups) {
        ...
        for (const group of groups) {
            ...
            if (start === -1) {
                ...
                const group_action_count = group._group_actions.length
                const end = start + group_action_count
                if (_array_equal(actions.slice(start, end), group._group_actions)) {

                    let suppressed_actions_count = 0
                    ...
                    const exposed_actions_count = group_action_count - suppressed_actions_count
...
const _ActionsContainer = _callable(class _ActionsContainer {
    ...
    constructor () {
        ...
        this._negative_number_matcher = /^-(?:\d+(?:_\d+)*(?:\.\d+(?:_\d+)*)?|\.\d+(?:_\d+)*)$/
...
const ArgumentParser = _callable(class ArgumentParser extends _AttributeHolder(_ActionsContainer) {
    ...
    _parse_known_args (arg_strings, namespace) {
        ...
        const consume_optional = start_index => {
            ...
            let [action, option_string, sep, explicit_arg] = option_tuple
        ...
        // separate
            ...
            for (const option_string of Object.keys(this._option_string_actions)) {
                if (option_string === short_option_prefix) {
                    ...
                    const action = this._option_string_actions[option_string]
                    const tup = [action, option_string, undefined, undefined]
                    result.push(tup)
                }
            }
    ...
    _get_values (action, arg_strings) {
        ...
        if (!action.option_strings.length && ![PARSER, REMAINDER].includes(action.nargs)) {
            try {

...
const HelpFormatter = _callable(class HelpFormatter {
    ...
    add_argument (action) {
        if (action.help !== SUPPRESS) {
            ...
            const invocation_lengths = [
                this._format_action_invocation(action).length + this._current_indent
            ]
            for (const subaction of this._iter_indented_subactions(action)) {
                invocation_lengths.push(
                    this._format_action_invocation(subaction).length + this._current_indent
                )
            }
    ...
    _format_usage (usage, actions, groups, prefix) {
        ...
        // if optionals and positionals are available, calculate usage
            ...
            if (prefix.length + usage.length > text_width) {
                ...
                const all_actions = optionals.concat(positionals)
                const [parts, pos_start] = this._get_actions_usage_parts_with_split(
                    all_actions, groups, optionals.length
                )
    ...
    _format_actions_usage (actions, groups) {
        return this._get_actions_usage_parts(actions, groups).join(' ')
    }

    _get_actions_usage_parts (actions, groups) {
        const [parts] = this._get_actions_usage_parts_with_split(actions, groups)
        return parts
    }

    _get_actions_usage_parts_with_split (actions, groups, opt_count = undefined) {
        /* Get usage parts with split index for optionals/positionals.

         * Returns [parts, pos_start] where pos_start is the index in parts
         * where positionals begin. When opt_count is undefined, pos_start is
         * undefined. This preserves mutually exclusive group formatting
         * across the optionals/positionals boundary (gh-75949).
         */
        ...
        for (const group of groups) {
            ...
            if (group._group_actions.every(action => action.help === SUPPRESS)) {
                continue
            }

            const indices = group._group_actions.map(item => actions.indexOf(item))
            if (indices.includes(-1)) {
                continue
            } else {
                const start = Math.min(...indices)
                const end = start + group._group_actions.length
                const action_set = new Set(actions.slice(start, end))
                if (action_set.size === group._group_actions.length &&
                    group._group_actions.every(action => action_set.has(action))) {
                    for (const action of group._group_actions) {
                        group_actions.add(action)
                    }
                    inserts.push([start, end, group])
                }
            }
        }
        ...
        const inserted_separators_indices = new Set()
        inserts.sort((a, b) => b[0] - a[0] || b[1] - a[1])
        for (const [start, end, group] of inserts) {
            const group_parts = parts.slice(start, end).filter(item => item !== undefined)
            const group_size = group_parts.length
            let open
            let close
            if (group.required) {
                [open, close] = group_size > 1 ? ['(', ')'] : ['', '']
            } else {
                [open, close] = ['[', ']']
            }
            group_parts[0] = open + group_parts[0]
            group_parts[group_parts.length - 1] += close
            for (let i = start; i < start + group_size - 1; i++) {
                // insert a separator if not already done in a nested group
                if (!inserted_separators_indices.has(i)) {
                    parts[i] = group_parts[i - start] + ' |'
                    inserted_separators_indices.add(i)
                }
            }
            parts[start + group_size - 1] = group_parts[group_parts.length - 1]
            for (const i of range(start + group_size, end)) {
                parts[i] = undefined
            }
        }
        ...
        // Count before filtering undefined entries since indices shift after.
        let pos_start
        if (opt_count !== undefined) {
            pos_start = parts.slice(0, opt_count).filter(part => part !== undefined).length
        }

        // return the usage parts and split point (gh-75949)
        return [parts.filter(item => item !== undefined), pos_start]
    }
...
const _deprecated_default = Symbol('deprecated_default')
...
const BooleanOptionalAction = _callable(class BooleanOptionalAction extends Action {
    ...
    constructor () {
        ...
        // We need _deprecated_default special value to ban explicit arguments
        // that match the default value. Like:
        //   parser.add_argument('-f', { action: BooleanOptionalAction, type: Number })
        for (const [field_name, value] of Object.entries({ type, choices, metavar })) {
            if (value !== _deprecated_default) {
                deprecate('BooleanOptionalAction.' + field_name,
                    sub('%r is deprecated as of Python 3.12 and will be removed in Python 3.14.',
                        field_name))
            }
        }

        const action_type = type === _deprecated_default ? undefined : type
        const action_choices = choices === _deprecated_default ? undefined : choices
        const action_metavar = metavar === _deprecated_default ? undefined : metavar
...
const _ActionsContainer = _callable(class _ActionsContainer {
    ...
    _get_positional_kwargs () {
        ...
        if (![OPTIONAL, ZERO_OR_MORE, REMAINDER, SUPPRESS, 0].includes(nargs)) {
...
const _ArgumentGroup = _callable(class _ArgumentGroup extends _ActionsContainer {
    ...
    add_argument_group () {
        deprecate('nested argument group', 'Nesting argument groups is deprecated.')
        return super.add_argument_group(...arguments)
    }
})
...
const _MutuallyExclusiveGroup = _callable(class _MutuallyExclusiveGroup extends _ArgumentGroup {
    ...
    add_mutually_exclusive_group () {
        deprecate('nested mutually exclusive group',
            'Nesting mutually exclusive groups is deprecated.')
        return super.add_mutually_exclusive_group(...arguments)
    }
})
...
const ArgumentParser = _callable(class ArgumentParser extends _AttributeHolder(_ActionsContainer) {
    ...
    add_subparsers () {
        ...
        if (this._subparsers !== undefined) {
            throw new ArgumentError(undefined, 'cannot have multiple subparser arguments')
        }
